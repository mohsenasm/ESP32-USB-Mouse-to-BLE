...
static const char *TAG2 = "usb-handler";
...
void (*send_mouse_callback)(uint8_t, int8_t, int8_t, int8_t);
...
static void hid_host_mouse_report_callback(const uint8_t *const data, const int length)
{
    ...
    // if (mdata->button == 0x1) {
    //     buttons = 1;
    // } else if (mdata->button == 0x2) {
    //     buttons = 2;
    // } else if (mdata->button == 0x4) {
    //     buttons = 4;
    // }
    buttons = mdata->button;
    if (mdata->x_displacement > INT8_MAX) {
        dx = INT8_MAX;
    } else if (mdata->x_displacement < INT8_MIN) {
        dx = INT8_MIN;
    } else {
        dx = (int8_t)mdata->x_displacement;
    }
    if (mdata->y_displacement > INT8_MAX) {
        ...
        dy = (int8_t)mdata->y_displacement;
    }
    dw = -mdata->wheel; // inverse scroll!

    send_mouse_callback(buttons, dx, dy, dw);
                                 ...
                                 void *arg)
{
    ...
    case HID_HOST_INTERFACE_EVENT_DISCONNECTED:
        ESP_LOGI(TAG2, "HID Device, protocol '%s' DISCONNECTED",
                 hid_proto_name_str[dev_params.proto]);
        ESP_ERROR_CHECK(hid_host_device_close(hid_device_handle));
        break;
    case HID_HOST_INTERFACE_EVENT_TRANSFER_ERROR:
        ESP_LOGI(TAG2, "HID Device, protocol '%s' TRANSFER_ERROR",
                 hid_proto_name_str[dev_params.proto]);
        break;
    default:
        ESP_LOGE(TAG2, "HID Device, protocol '%s' Unhandled event",
                 hid_proto_name_str[dev_params.proto]);
        break;
    }
}
                           ...
                           void *arg)
{
    ...
    ESP_LOG_BUFFER_HEX(TAG2, (const void*)&dev_params, sizeof(dev_params));

    switch (event) {
    case HID_HOST_DRIVER_EVENT_CONNECTED:
        ESP_LOGI(TAG2, "HID Device, protocol '%s' CONNECTED",
                 hid_proto_name_str[dev_params.proto]);
...
void init_usb(void (*_send_mouse_callback)(uint8_t, int8_t, int8_t, int8_t))
{
    send_mouse_callback = _send_mouse_callback;

    BaseType_t task_created;
    app_event_queue_t evt_queue;
    ESP_LOGI(TAG2, "HID Host example");
    ...
    */
    ...
    ESP_LOGI(TAG2, "Waiting for HID Device to be connected");
        ...
        if (xQueueReceive(app_event_queue, &evt_queue, portMAX_DELAY)) {
            if (APP_EVENT == evt_queue.event_group) {
                ...
                if (lib_info.num_devices == 0) {
                    ...
                    ESP_LOGW(TAG2, "To shutdown example, remove all USB devices and press button again.");
                    // Keep polling
                }
            }
    ...
    ESP_LOGI(TAG2, "HID Driver uninstall");
